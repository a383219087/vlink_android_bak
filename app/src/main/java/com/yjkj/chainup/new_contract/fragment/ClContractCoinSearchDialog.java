 ...
 * 合约币种搜索对话框
 */
public class ClContractCoinSearchDialog extends NBaseDialogFragment implements WsContractAgentManager.WsResultCallback {
...
//        setStyle(DialogFragment.STYLE_NORMAL, R.style.leftin_rightout_DialogFg_style);
    ...
    protected int setContentView() {
        return R.layout.cl_dialog_contract_coin_search;
    }
...
//        findViewById(R.id.alpha_ll).setOnClickListener(this);
    ...
    protected void loadData() {
        WsContractAgentManager.Companion.getInstance().addWsCallback(this);
    ...
    private void initTab() {
        ...
        for (int i = 0; i < mContractList.length(); i++) {
            try {
                LogUtil.e("合约币对", mContractList.getJSONObject(i).get("symbol").toString());
                // (反向：0，1：正向 , 2 : 混合 , 3 : 模拟)
                LogUtil.e("合约方向", mContractList.getJSONObject(i).get("contractSide").toString());
        ...
        rmap.put("bind", true);
        rmap.put("symbols", JsonUtils.gson.toJson(arrays));
...
//        WsContractAgentManager.instance.sendMessage(hashMapOf("bind" to true, "symbols" to JsonUtils.gson.toJson(arrays)), this)
        ...
        //USDT
        if (isHasU) {
            showTitles.add(LanguageUtil.getString(getContext(), "cl_market_text4"));
            fragments.add(ClCoinSearchItemFragment.newInstance(1, contractListJson));
        }
        //币本位
        if (isHasB) {
            showTitles.add(LanguageUtil.getString(getContext(), "cl_market_text5"));
            fragments.add(ClCoinSearchItemFragment.newInstance(0, contractListJson));
        }
        //混合
        if (isHasH) {
            showTitles.add(LanguageUtil.getString(getContext(), "cl_market_text6"));
            fragments.add(ClCoinSearchItemFragment.newInstance(2, contractListJson));
        }
        //模拟
        if (isHasM) {
            showTitles.add(LanguageUtil.getString(getContext(), "cl_market_text7"));
            fragments.add(ClCoinSearchItemFragment.newInstance(3, contractListJson));
        }
        ...
        tl_market_aa.setOnTabSelectListener(new OnTabSelectListener() {
            ...
            public void onTabSelect(int position) {
                LogUtil.e(TAG,"点击的position"+position);
                if (showTitles.get(position).equals(LanguageUtil.getString(getContext(), "cl_market_text7"))){
                    EventBusUtil.post(new  MessageEvent(MessageEvent.sl_contract_receive_coupon));
                }
            }
    ...
    public void onWsMessage(@NotNull String json) {
        try {
            ...
            MessageEvent messageEvent = new MessageEvent(MessageEvent.sl_contract_sidebar_market_event);

...
class NewVersionHomePageViewModel : HomePageViewModel() {
        ...
        override fun onClick(item: Item) {
            ...
            when (item.index.value) {
                ...
                3 -> {
                    ...
                    val url =
                        "${ChainUpApp.url?.optionUrl}?token=${token}&lang=${lang}&type=${style}"
                    val bundle = Bundle()
        ...
        if (PublicInfoDataService.getInstance().isEnforceGoogleAuth(null)) {
            if (UserDataService.getInstance().googleStatus != 1) {
                NewDialogUtils.OTCTradingMustPermissionsDialog(
                    ...
                    type = type,
                    title = LanguageUtil.getString(mActivity.value, "withdraw_tip_bindGoogleFirst")
                )
                return true
            }
        } else {
            if (UserDataService.getInstance().isOpenMobileCheck != 1 && UserDataService.getInstance().googleStatus != 1) {
                NewDialogUtils.OTCTradingPermissionsDialog(
                    ...
                    title = LanguageUtil.getString(
                        mActivity.value,
                        "otcSafeAlert_action_bindphoneOrGoogle"
                    )
                )
                return true
            }
    ...
    val itemBinding =
        ItemBinding.of<Item>(BR.item, R.layout.item_new_version_item)
...
//    "share":1
    fun getPublicInfo(context: Context) {
        ...
        startTask(
            apiService.getPublicInfo1(toRequestBody(DataHandler.encryptParams(map))),
            Consumer {
                ...
                if (recharge == 1) {
                    ...
                    item.title.value =
                        LanguageUtil.getString(context, "NewVersionHomePageViewModel_text1")
                    items.add(item)
                }
                /**
                 * 提现转账
                ...
                if (withdraw == 1) {
                    ...
                    item.title.value =
                        LanguageUtil.getString(context, "NewVersionHomePageViewModel_text2")
                    items.add(item)
                }
                /**
                 * 合约
                ...
                if (contract == 1) {
                    ...
                    item.title.value =
                        LanguageUtil.getString(context, "NewVersionHomePageViewModel_text3")
                    items.add(item)
                }
                /**
                 * 期权
                ...
                if (futures == 1) {
                    ...
                    item.title.value =
                        LanguageUtil.getString(context, "NewVersionHomePageViewModel_text4")
                    items.add(item)
                }
                /**
                 * 跟单
                ...
                if (trader == 1) {
                    ...
                    item.title.value =
                        LanguageUtil.getString(context, "NewVersionHomePageViewModel_text5")
                    items.add(item)
                }
                /**
                 * 理财
                ...
                if (increment == 1) {
                    ...
                    item.title.value =
                        LanguageUtil.getString(context, "NewVersionHomePageViewModel_text6")
                    items.add(item)
                }
...
//            }
                ...
                item.title.value =
                    LanguageUtil.getString(context, "NewVersionHomePageViewModel_text114")
                items.add(item)
    ...
    fun homeRecharge() {
        ...
        if (PublicInfoDataService.getInstance().depositeKycOpen && UserDataService.getInstance().authLevel != 1) {
            NewDialogUtils.KycSecurityDialog(
                mActivity.value!!,
                mActivity.value!!.getString(R.string.common_kyc_chargeAndwithdraw),
                object : NewDialogUtils.DialogBottomListener {
                    override fun sendConfirm() {
                        when (UserDataService.getInstance().authLevel) {
                            0 -> {
                                NToastUtil.showTopToastNet(
                                    mActivity.value,
                                    false,
                                    mActivity.value!!.getString(R.string.noun_login_pending)
                                )
                            }
    ...
    fun withdrawalTransfer() {
        ...
        if (PublicInfoDataService.getInstance().withdrawKycOpen && UserDataService.getInstance().authLevel != 1) {
            NewDialogUtils.KycSecurityDialog(mActivity.value!!,
                mActivity.value?.getString(R.string.common_kyc_chargeAndwithdraw)
                    ?: "",
                object : NewDialogUtils.DialogBottomListener {
                    override fun sendConfirm() {

...
class SlPlCalculatorFragment : BaseCalculatorFragment(){
    ...
    override fun doCalculator() {
        ...
        val marginValue = "<font color=\"#FF9E12\">${dfNormal.format(margin)} </font> ${contract?.showMarginName()}"
        val tabList = ArrayList<TabInfo>()
        tabList.add(TabInfo(getLineText("sl_str_take_up_margin"),marginValue))
        tabList.add(TabInfo(getLineText("sl_str_position_value"),dfNormal.format(MathHelper.round(value, contract!!.value_index)) + contract!!.showMarginName()))
        tabList.add(TabInfo(getLineText("sl_str_pl"),dfNormal.format(MathHelper.round(profitAmount, contract!!.value_index)) + contract?.showMarginName()))

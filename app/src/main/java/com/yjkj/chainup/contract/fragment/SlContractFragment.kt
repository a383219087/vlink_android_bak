 ...
 * 合约
 */
class SlContractFragment : NBaseFragment(), SwipeRefreshLayout.OnRefreshListener, SLDoListener {

    //tab类型  0 开仓  1 平仓  2 持仓
    private var currentIndex = 0
    private lateinit var tradeFragment: SlContractTradeFragment
    private lateinit var holdFragment: SlContractHoldFragment
    ...
    fun createContractAccount() {
        ...
        if (ContractCloudAgent.isCloudOpen) {
            val uid = ContractSDKAgent.user.uid ?: ""
            val token = ContractSDKAgent.user.token ?: ""
            if (uid.isEmpty() || token.isEmpty()) {
                //初始化合约子账户
                val userInfo4UserId = UserDataService.getInstance().userInfo4UserId
                ContractCloudAgent.init(userInfo4UserId, object : SLDoListener {
                    override fun doThing(obj: Any?): Boolean {
                        ContractUserDataAgent.loadContractAccount(object : IResponse<MutableList<ContractAccount>>() {
                            override fun onSuccess(data: MutableList<ContractAccount>) {

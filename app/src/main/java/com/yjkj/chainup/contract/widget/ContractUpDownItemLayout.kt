...
class ContractUpDownItemLayout : LinearLayout {
    ...
    private fun initAttrs(attrs: AttributeSet?) {
        attrs?.let {
            ...
            typedArray.recycle()
            if (showExplain) {
                setExplainListener(OnClickListener { NewDialogUtils.showDialog(context, explainText, true, null, context.getString(R.string.common_text_tip), context.getString(R.string.alert_common_i_understand)) })
            }
        }
    }
    ...
    var showExplain = false
        set(value) {
            field = value
            if (showExplain) {
                tv_title?.compoundDrawablePadding = SizeUtils.dp2px(3f)

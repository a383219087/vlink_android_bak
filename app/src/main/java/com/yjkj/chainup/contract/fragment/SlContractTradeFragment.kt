...
class SlContractTradeFragment : NBaseFragment(), BuySellContractAdapter.OnBuySellContractClickedListener, LogicContractSetting.IContractSettingListener, SLDoListener {
    ...
    fun bindContract(contract: Contract, resetData: Boolean = false) {
        ...
        //设置过滤监听
        et_trigger_price.numberFilter(mContract?.price_index!! - 1, otherFilter = updateAvailableVolListener)
        et_price.numberFilter(mContract?.price_index!! - 1, otherFilter = updatePriceListener)
        ...
        if (resetData) {
            ...
            ContractPublicDataAgent.loadDepthFromNet(mContract?.instrument_id!!, getDepthSubCount())
        }
        //限价委托
    ...
    private val updatePriceListener = object : SLDoListener {
        override fun doThing(obj: Any?): Boolean {
            mContract?.let {
                ...
                tv_price_value.text = RateManager.getCNYByCoinName(it.quote_coin, price, precision = it.value_index)
                updateAvailableVol()
            }
            return true
        }
    ...
    private val updatePositionListener = object : SLDoListener {
        override fun doThing(obj: Any?): Boolean {
            mContract?.let {
                val vol: String = et_position.text.toString()
                ...
                if (vol.isEmpty()) {
                    val value: String = ContractUtils.CalculateContractBasicValue(
                            ContractCalculate.trans2ContractVol(it, "0", "0", unit),
                            "0",
                            it)
                    ...
                    if (GlobalLeverageUtils.isOpenGlobalLeverage) {
                        if (LoginManager.checkLogin(activity, true)) {
                            if (ContractUserDataAgent.hasPositionOrOrder(it.instrument_id)) {
                                ToastUtils.showToast("contract_leavgerLimit".localized(activity!!))
                            } else {
    ...
    fun updateAvailableVol() {
        ...
        mContract?.let {
            ...
            if (buyOrSellHelper.tradeType == 1) {//平仓
                ...
                //计算预估成本
                val buyCost = calculateAdvanceOpenCost(true)
                val sellCost = calculateAdvanceOpenCost(false)
                val dfDefault = NumberUtil.getDecimal(it.value_index)
                if (buyCost == null || !ContractSDKAgent.isLogin) {
                    tv_buy_cost.text = " --"
                } else {
                    tv_buy_cost.text = " " + dfDefault.format(MathHelper.round(buyCost.freezAssets, it.value_index)) + " " + it.showMarginName()
                }
                if (sellCost == null || !ContractSDKAgent.isLogin) {
                    tv_sell_cost.text = " --"
                } else {
                    tv_sell_cost.text = " " + dfDefault.format(MathHelper.round(sellCost.freezAssets, it.value_index)) + " " + it.showMarginName()
                }
            }
        }
    }
     ...
     * 计算预估成本
     */
    private fun calculateAdvanceOpenCost(isBuy: Boolean): AdvanceOpenCost? {
        mContract?.let {
            ...
            contractOrder.qty = ContractCalculate.trans2ContractVol(it, etPosition, etPrice, LogicContractSetting.getContractUint(ContractSDKAgent.context))
    ...
    fun updateContractTicker(ticker: ContractTicker, updatePrice: Boolean = true) {
        ...
        mContract?.let {
            ...
            iv_funds_rate_value.text = NumberUtil.getDecimal(4).format(MathHelper.round(rate, 4)).toString() + "%"
            if (updatePrice) {
                et_price.setText(dfprice1.format(current))
            }
            //展示币种类型
            val unit = LogicContractSetting.getContractUint(ContractSDKAgent.context)
            tv_amount.text = getLineText("sl_str_amount") + "(" + (if (unit == 0) getLineText("sl_str_contracts_unit") else it.showBaseName()) + ")"
        }
    }
    ...
    fun updateUserAssetUI() {
        ...
        if (contractAccount == null) {
            tv_aavl_value.text = "0 ${mContract!!.showMarginName()}"
            return
        }
        val dfValue = NumberUtil.getDecimal(mContract!!.value_index)
        val available = contractAccount.available_vol_real
        tv_aavl_value.text = dfValue.format(available) + " " + mContract!!.showMarginName()
    }
    ...
    private fun doBuyOrSell() {
        mContract?.let { contract ->
            ...
            if (cAccount == null) {
                val tips = java.lang.String.format(getLineText("sl_str_no_contract_account"), contract.showMarginName())
            ...
            if (buyOrSellHelper.priceType != CONTRACT_ORDER_PLAN) {
                ...
                if (buyOrSellHelper.tradeType == 0) {
                    ...
                    window.setOkListener(object : SLDoListener {
                        override fun doThing(obj: Any?): Boolean {
                            window.dismiss()
                    ...
                    val title = if (buyOrSellHelper.priceType == CONTRACT_ORDER_MARKET) {
                        "contract_action_marketClosing".localized(activity!!)
                    } else {
                        "contract_text_limitPositions".localized(activity!!)
                    }
                    val tt = if (buyOrSellHelper.priceType == CONTRACT_ORDER_MARKET) {
                        "contract_action_marketPrice".localized(activity!!)
                    } else {
                        "contract_action_limitPrice".localized(activity!!)
                    }
                    val side: String = if (buyOrSellHelper.isBuy) {
                        "contract_action_buy".localized(activity!!)
                    } else {
                        "contract_action_sell".localized(activity!!)
                    }
                    val content = if (buyOrSellHelper.priceType == CONTRACT_ORDER_MARKET) {
                        tt + side + vol + "sl_str_contracts_unit".localized(activity!!) + contract.showBaseName() + contract.showMarginName() + "trade_contract_title".localized(activity!!)
                    } else {
                        tt + etPrice + contract.showMarginName() + side + vol + "sl_str_contracts_unit".localized(activity!!) + contract.showBaseName() + contract.showMarginName() + "trade_contract_title".localized(activity!!)
                    }
            ...
            val response: IResponse<String> = object : IResponse<String>() {
                ...
                override fun onFail(code: String, msg: String) {
                    ...
                    if (code == "CONTRACT_LEVERAGE_MATCH_ERROR" || code == "LEVERAGE_MATCH_ERROR") {
                        ...
                        NToastUtil.showToast(getLineText("contract_string_leverageerror"), false)
                    } else {

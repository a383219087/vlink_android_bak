...
class ContractIntroduceFragment : NBaseFragment(){
    ...
    override fun initView() {
        ...
        contract?.let {
            //合约标的
            tv_contract_underlying.text = it.showBaseName()
            //保证金币种
            tv_margin_coin.text =it.showMarginName()
            //合约属性
            tv_contract_property.text = if (it.isReserve) getLineText("sl_str_reserve_contract") else getLineText("sl_str_positive_contract")
            //合约大小
            tv_contract_size.text = "1" + getLineText("sl_str_contracts_unit") + "=" + it.face_value + it.showPriceName()
            //最高杠杆

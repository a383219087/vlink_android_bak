...
class ContractPlanEntrustAdapter(context: Context, data: ArrayList<ContractOrder>) : BaseQuickAdapter<ContractOrder, BaseViewHolder>(R.layout.sl_item_contract_plan_entrust, data) {
    ...
    init {
        ...
        sl_str_trigger_failed = context.getLineText("sl_str_trigger_failed")
    }
    ...
    override fun convert(helper: BaseViewHolder, item: ContractOrder) {
        ...
        helper?.run {
            ...
            //触发价格
            setText(R.id.tv_trigger_price,sl_str_trigger_price +" (" +contract.showQuoteName() +")")
            ...
            var executionPriceTitle = sl_str_execution_price +" (" +contract.showQuoteName() +")"
            ...
            if(isCurrentEntrust){
                ...
                //隐藏触发时间
                setGoneV3(R.id.rl_deal_price,false)
            }else{
                ...
                //触发时间
                setGoneV3(R.id.rl_deal_price,true)
            ...
            if(isCurrentEntrust){
                ...
                setGoneV3(R.id.tv_cancel,true)
                setGoneV3(R.id.tv_status,false)
                ...
                setGoneV3(R.id.tv_cancel,false)
    ...
    private fun queryDetail(order: ContractOrder) {
        ...
        when {
            order.trigger_type === 1 -> {
                price_type = context.getLineText("sl_str_latest_price_simple")
            }
            order.trigger_type === 2 -> {
                price_type = context.getLineText("sl_str_fair_price_simple")
            }
            order.trigger_type === 4 -> {
                price_type = context.getLineText("sl_str_index_price_simple")
            }
        }
        var reason: String
        var content: String
        when (order.errno) {
            3 -> {
                reason = context.getLineText("sl_str_trigger_failed_reason3")
                content = java.lang.String.format(context.getLineText("sl_str_trigger_failed_info"),
                        time,
                        contract.getDisplayName(context), price_type,
                        dfDefault.format(MathHelper.round(order.px, contract.price_index)) + contract.showQuoteName(),
                        reason)
            }
            4 -> {
                reason = context.getLineText("sl_str_trigger_failed_reason4")
                content = java.lang.String.format(context.getLineText("sl_str_trigger_failed_info"),
                        time,
                        contract.getDisplayName(context), price_type,
                        dfDefault.format(MathHelper.round(order.px, contract.price_index)) + contract.showQuoteName(),
            ...
            5 -> content = context.getLineText("sl_str_trigger_failed_reason5")
            6 -> {
                reason = context.getLineText("sl_str_trigger_failed_reason6")
                content = java.lang.String.format(context.getLineText("sl_str_trigger_failed_info"),
                        time,
                        contract.getDisplayName(context), price_type,
                        dfDefault.format(MathHelper.round(order.px, contract.price_index)) + contract.showQuoteName(),
            ...
            7 -> {
                reason = context.getLineText("sl_str_trigger_failed_reason7")
                content = java.lang.String.format(context.getLineText("sl_str_trigger_failed_info"),
                        time,
                        contract.getDisplayName(context), price_type,
                        dfDefault.format(MathHelper.round(order.px, contract.price_index)) + contract.showQuoteName(),
            ...
            8 -> {
                reason = context.getLineText("sl_str_trigger_failed_reason8")
                content = java.lang.String.format(context.getLineText("sl_str_trigger_failed_info"),
                        time,
                        contract.getDisplayName(context), price_type,
                        dfDefault.format(MathHelper.round(order.px, contract.price_index)) + contract.showQuoteName(),
            ...
            9 -> {
                reason = java.lang.String.format(context.getLineText("sl_str_trigger_failed_reason9"),order.pid.toString())
                content = java.lang.String.format(context.getLineText("sl_str_trigger_failed_info"),
                        time,
                        contract.getDisplayName(context), price_type,
                        dfDefault.format(MathHelper.round(order.px, contract.price_index)) + contract.showQuoteName(),
            ...
            10 -> {
                reason = context.getLineText("sl_str_trigger_failed_reason10")
                content = java.lang.String.format(context.getLineText("sl_str_trigger_failed_info"),
                        time,
                        contract.getDisplayName(context), price_type,
                        dfDefault.format(MathHelper.round(order.px, contract.price_index)) + contract.showQuoteName(),
            ...
            11 -> content = context.getLineText("sl_str_trigger_failed_reason11")
            12 -> {
                reason = context.getLineText("sl_str_trigger_failed_reason12")
                content = java.lang.String.format(context.getLineText("sl_str_trigger_failed_info"),
                        time,
                        contract.getDisplayName(context), price_type,
                        dfDefault.format(MathHelper.round(order.px, contract.price_index)) + contract.showQuoteName(),
            ...
            13 -> content = context.getLineText("sl_str_trigger_failed_reason13")
            else -> content = context.getLineText("sl_str_trigger_failed_reason13")
        }

        NewDialogUtils.showDialog(context,content,true,null,context.getLineText("sl_str_trigger_failed"))
    }

...
class SlContractAssetRecordActivity : NBaseActivity() {
    ...
    override fun loadData() {
        ...
        if (contractAccounts != null && contractAccounts.isNotEmpty()) {
            for (index in contractAccounts.indices) {
                if (TextUtils.equals(contractAccounts[index].coin_code, symbol)) {
                    mCurrContractInfo = TabInfo(NCoinManager.getShowMarket(contractAccounts[index].coin_code), index)
                }
                contractList.add(TabInfo(NCoinManager.getShowMarket(contractAccounts[index].coin_code), index))
            }
        } else {
        ...
        typeList.add(TabInfo(getLineText("sl_str_transfer_bb2contract"), getContractTypeIndex(true)))
        typeList.add(TabInfo(getLineText("sl_str_transfer_contract2bb"), getContractTypeIndex(false)))
...
//        }
    ...
    private fun loadDataFromNet() {
        val action: IntArray? = getTypeAction()
        if (mOffset == 0) {
            showLoadingDialog()
        }
        isLoading = true
...
//                    mOffset = mList.size
                ...
                assetAdapter?.notifyDataSetChanged()
                assetAdapter?.loadMoreModule?.checkDisableLoadMoreIfNotFullPage()
            }
     ...
     * 获取合约转入转出类型
     */
    private fun getContractTypeIndex(isIn: Boolean): Int {
        return if (ContractSDKAgent.isContractCloudSDK) {
            if (isIn) 22 else 23
        } else {
            if (isIn) 5 else 6
        }

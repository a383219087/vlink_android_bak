...
class ContractTradeConfirmWindow(private val context: Context, private val buyOrSellHelper: SLContractBuyOrSellHelper) : PopupWindow() {
     ...
     * 设置确定和取消事件
     */
    fun setOkListener(okListener: SLDoListener?) {
        btnOk.setOnClickListener { doSubmit(okListener) }
        btnOkTwo.clicked = true
        btnOkTwo.rl_layout.setOnClickListener { doSubmit(okListener) }
    }

    /**
     * 提交
     */
    private fun doSubmit(okListener: SLDoListener?) {
    ...
    private fun initData() {
        buyOrSellHelper.contract?.let { contract ->
            ...
            val priceStr = "${context.getLineText("contract_text_price")}(${contract.showQuoteName()})"
            ...
            val code = contract.showMarginName()
            val dfDefault = NumberUtil.getDecimal(-1)
            val dfValue = NumberUtil.getDecimal(buyOrSellHelper.contract?.value_index!!)
            val contractValue = ContractCalculate.CalculateContractValue(contractOrder?.qty!!, buyOrSellHelper.etPrice!!, contract)
            val entrust = dfValue.format(MathHelper.round(contractValue, contract.value_index)) + " " + code
            ...
            val cost = (if (longOpenCost == null) "0 $code" else dfValue.format(MathHelper.round(longOpenCost.freezAssets, contract.value_index))) + " " + code
            tvCost.text = cost
    ...
    private fun buildStopRateLoss() {
        ...
        item_stop_rate.apply {
            ...
            priceUnit = buyOrSellHelper.contract!!.showMarginName()
    ...
    private fun doSubmitPreVerify(): Boolean {
        if (isRateOpen) {
            if (MathHelper.round(item_stop_rate.triggerPrice) <= 0) {
                NToastUtil.showTopToastNet(getActivity(),false, context.getLineText("sl_str_stop_rate") + context.getLineText("sl_str_price_too_low"))
                return false
            }
        ...
        if (isLossOpen) {
            if (MathHelper.round(item_stop_loss.triggerPrice) <= 0) {
                NToastUtil.showTopToastNet(getActivity(),false, context.getLineText("sl_str_stop_loss") + context.getLineText("sl_str_price_too_low"))
                return false
            }

...
class ContractPriceEntrustAdapter(context:Context,data: ArrayList<ContractOrder>) : BaseQuickAdapter<ContractOrder, BaseViewHolder>(R.layout.sl_item_contract_price_entrust, data) {
    ...
    override fun convert(helper: BaseViewHolder, item: ContractOrder) {
        ...
        helper?.run {
            ...
            itemEntrustPrice.title = sl_str_entrust_price_unit + "(" + contract.showQuoteName() + ")"
//            //成交均价
//            setText(R.id.tv_deal_price_value, dfDefault.format(MathHelper.round(item.avg_px, contract.price_index)))
//            setText(R.id.tv_deal_price, context.getLineText(R.string.contract_text_dealAverage) +" (" + contract.quote_coin+")")
            //委托价值
            ...
            val value: Double = ContractCalculate.CalculateContractValue(
                    ...
                    item.px,
                    contract)
            itemEntrustValue.title =  String.format(sl_str_entrust_value_unit, contract.showMarginName())
            if (!isCurrentEntrust && isForceCloseOrder) {
                itemEntrustValue.content = "--"
            } else {
                itemEntrustValue.content =  dfValue.format(MathHelper.round(value, contract.value_index))
            }
            //时间
            ...
            if (isCurrentEntrust) {
                ...
                tvCancel.setOnClickListener { v ->
                    NewDialogUtils.showDialog(context!!, context.getLineText("sl_str_cancel_order_tips"), false, object : DialogBottomListener {
                        override fun sendConfirm() {
                            doCancel(v, item, "")
                        }
                    }, context.getLineText("common_text_tip"), context.getLineText("common_text_btnConfirm"), context.getLineText("common_text_btnCancel"))
                }
            } else {
                ...
                getView<TextView>(R.id.tv_order_type).setOnClickListener { v ->
                    SlContractEntrustDetailActivity.show(context as Activity, item)
                }
            }
    ...
    private fun doDealSystemCanceled(type:Int,textView: TextView,isShow:Boolean){
        ...
        if(isShow && !TextUtils.isEmpty(cancelString)){
            textView.setCompoundDrawablesWithIntrinsicBounds(null,null,context.resources.getDrawable(R.drawable.sl_contract_prompt),null)
            textView.setOnClickListener {
                NewDialogUtils.showSingleDialog(context, cancelString, object : NewDialogUtils.DialogBottomListener {
                    override fun sendConfirm() {

                    }

                }, context.getLineText("common_text_tip"), context.getLineText("alert_common_iknow"))
            }
        }else{
    ...
    private fun getSystemCanceledFormat(type:Int) : String{
        return when(type){
            2 -> context.getLineText("sl_str_entrust_failed_reason2")
            3 -> context.getLineText("sl_str_entrust_failed_reason3")
            4 -> context.getLineText("sl_str_entrust_failed_reason4")
            5 -> context.getLineText("sl_str_entrust_failed_reason5")
            6 -> context.getLineText("sl_str_entrust_failed_reason6")
            7 -> context.getLineText("sl_str_entrust_failed_reason7")
            8 -> context.getLineText("sl_str_entrust_failed_reason8")
            9 -> context.getLineText("sl_str_entrust_failed_reason9")
            10 -> context.getLineText("sl_str_entrust_failed_reason10")
            11 -> context.getLineText("sl_str_entrust_failed_reason11")
            12 -> context.getLineText("sl_str_entrust_failed_reason12")
            13 -> context.getLineText("sl_str_entrust_failed_reason13")
            14 -> context.getLineText("sl_str_entrust_failed_reason14")
            15 -> context.getLineText("sl_str_entrust_failed_reason15")
            16 -> context.getLineText("sl_str_entrust_failed_reason16")
            17 -> context.getLineText("sl_str_entrust_failed_reason17")
            18 -> context.getLineText("sl_str_entrust_failed_reason18")
            19 -> context.getLineText("sl_str_entrust_failed_reason19")
            else -> ""
        }
    }
            ...
            override fun onSuccess(data: List<ContractLiqRecord>) {
                if (data != null && data.isNotEmpty()) {
                    ...
                    if (order.side === ContractOrder.CONTRACT_ORDER_WAY_BUY_CLOSE_SHORT) {
                        priceChange = context.getLineText("sl_str_rose")
                        positionName = contractName + "-" + context.getLineText("sl_str_buy_close")
                    } else if (order.side === ContractOrder.CONTRACT_ORDER_WAY_SELL_CLOSE_LONG) {
                        priceChange = context.getLineText("sl_str_fall")
                        positionName = contractName + "-" + context.getLineText("sl_str_sell_close")
                    }
                    ...
                    if (type == 1) {    //部分强平
                        //部分强平
                        val intro1 = String.format(context.getLineText("sl_str_wiped_out_tips0"),
                                createdAt,
                                contractName,
                                priceChange + dfDefault.format(MathHelper.round(liqRecord.trigger_px, contract.price_index)) + contract.showQuoteName(),
                                positionName, MathHelper.round(MathHelper.mul(liqRecord.mmr, "100"), 2).toString() + "%")
                        val intro2 = java.lang.String.format(context.getLineText("sl_str_wiped_out_tips2"),
                                liqRecord.order_px + contract.showQuoteName())
                        SlDialogHelper.showWipedOutIntroduceDialog(context, context.getLineText("sl_str_force_close_details"), intro1, intro2, context.getLineText("sl_str_isee"), object : DialogBottomListener {
                            override fun sendConfirm() {
                                //  UtilSystem.openHtmlActivity(BTConstants.BTURL_CONTRACT_FORCE_CLOSE, context?.getLineText(R.string.sl_str_force_close_details))
                            }
                        })
                    } else if (type == 2 || type == 3) { //爆仓
                        //爆仓
                        val intro1 = String.format(context.getLineText("sl_str_wiped_out_tips1"),
                                createdAt,
                                contractName,
                                priceChange + dfDefault.format(MathHelper.round(liqRecord.trigger_px, contract.price_index)) + contract.showQuoteName(),
                                positionName, MathHelper.round(MathHelper.mul(liqRecord.mmr, "100"), 2).toString() + "%")
                        val intro2 = java.lang.String.format(context.getLineText("sl_str_wiped_out_tips2"),
                                liqRecord.order_px + contract.showQuoteName())
                        SlDialogHelper.showWipedOutIntroduceDialog(context, context.getLineText("sl_str_blowing_up_datail"), intro1, intro2, context.getLineText("sl_str_isee"), object : DialogBottomListener {
                            override fun sendConfirm() {
                                //  UtilSystem.openHtmlActivity(BTConstants.BTURL_CONTRACT_FORCE_CLOSE, context?.getLineText(R.string.sl_str_force_close_mechanism))
                            }
                        })
                    } else if (type == 4) {//减仓明细
                        if (category == 513) {
                            if (MathHelper.round(order.take_fee) > 0) {
                                ...
                                val intro1 = java.lang.String.format(context.getLineText("sl_str_reduce_position_tips"),
                                        createdAt,
                                        dfDefault.format(MathHelper.round(liqRecord.trigger_px, contract.price_index)) + contract.showQuoteName(),
                                        dfDefault.format(MathHelper.round(liqRecord.order_px, contract.price_index)) + contract.showQuoteName())

                                SlDialogHelper.showWipedOutIntroduceDialog(context, context.getLineText("sl_str_reduce_position_details"), intro1, "", context.getLineText("sl_str_isee"), object : DialogBottomListener {
                                    override fun sendConfirm() {
                                        //  UtilSystem.openHtmlActivity(BTConstants.BTURL_CONTRACT_AUTO_REDUCE, context?.getLineText(R.string.sl_str_automatically_reduce))
                                    }
                                })
                            }
                        } else {
    ...
    private fun doCancel(view: View, order: ContractOrder?, pwd: String) {
        ...
        if (loadDialog == null) {
            loadDialog = NLoadingDialog(context as Activity?)
        }
        ...
        val response: IResponse<MutableList<Long>> = object : IResponse<MutableList<Long>>() {
            ...
            override fun onSuccess(data: MutableList<Long>) {
                ...
                if (data != null && data.isNotEmpty()) {
                    ToastUtils.showToast(context, context.getLineText("sl_str_some_orders_cancel_failed"))
                    return
                }
                ToastUtils.showToast(context, context.getLineText("sl_str_cancel"))
            }

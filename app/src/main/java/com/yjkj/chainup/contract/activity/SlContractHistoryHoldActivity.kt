...
//                    mOffset = mList.size
                    ...
                    historyHoldAdapter?.loadMoreModule?.isEnableLoadMore = true
                    historyHoldAdapter?.loadMoreModule?.loadMoreComplete()
                } else {
                    historyHoldAdapter?.loadMoreModule?.loadMoreEnd()
                }
                LogUtil.d("DEBUG", "仓位数量:" + mList.size)
                historyHoldAdapter?.notifyDataSetChanged()
                historyHoldAdapter?.loadMoreModule?.checkDisableLoadMoreIfNotFullPage()
            }

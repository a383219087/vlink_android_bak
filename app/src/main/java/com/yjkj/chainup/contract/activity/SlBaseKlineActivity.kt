  ...
  */
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        ContractPublicDataAgent.registerKlineWsListener(this, object : ContractKlineListener() {
             ...
             * 更新某一条
             */
            override fun onWsContractKlineChange(type: ContractWsKlineType, contractId: Int, data: KLineData) {
                if (contractId == this@SlBaseKlineActivity.contractId && TextUtils.equals(timeMap[curTime]?.type, type.type)) {
             ...
             * 新增
             */
            override fun onWsContractKlineAdd(type: ContractWsKlineType, contractId: Int, data: ArrayList<KLineData>) {
                if (contractId == this@SlBaseKlineActivity.contractId && TextUtils.equals(timeMap[curTime]?.type, type.type)) {
...
//                loadKlineDataFromNet(forward = false, cleanCache = false)
    ...
    protected fun loadKlineDataFromNet(forward: Boolean = true, cleanCache: Boolean = false) {
        ...
        DataKLineHelper.loadKLineData(contractId, timeMap[curTime]
            ...
            override fun onLoadSplitData(id: Int, time: ContractWsKlineType, moreData: ArrayList<KLineData>?) {
                if (contractId == id && TextUtils.equals(time.type, timeMap[curTime]?.type)) {
    ...
    private fun initKLineData(data: List<KLineBean>?) {
        ...
        v_kline?.startAnimation()
        if (v_kline?.minScrollX != 0) {
            if (klineData.size < 30) {
                v_kline?.scrollX = 0
            } else {
                v_kline?.scrollX = v_kline?.minScrollX ?: 0
            }
        } else {
            Handler().postDelayed({
                v_kline?.scrollX = v_kline?.minScrollX ?: 0

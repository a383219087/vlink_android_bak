...
class SlContractStopRateLossActivity : NBaseActivity() {
     ...
     * 止盈止损是否可以下单
     */
    private var isRateLossEnable = true
    ...
    override fun loadData() {
        super.loadData()
        mPosition = intent.getParcelableExtra("position") ?: ContractPosition()
    ...
    private fun doSubmitPreVerify(isStopRateOrder: Boolean, itemLayout: SlPositionStopRateLossItem): ContractOrder? {
        ...
        if (MathHelper.round(triggerPrice) <= 0) {
            isRateLossEnable = false
            NToastUtil.showTopToastNet(mActivity, false, preTipsString + getLineText("sl_str_price_too_low"))
            return null
        }
        //非市价才校验执行价格
    ...
    private fun doSubmit(info: ContractOrder) {
        ...
        val opv = if (mPosition.side == 1) {
            MathHelper.roundUp(mPosition.avg_cost_px, contract!!.price_index - 1)
        } else {
            MathHelper.round(mPosition.avg_cost_px, contract!!.price_index - 1)
        }
        val warnPx = ContractCalculate.calculateStopLostWarnPx(contract!!, opv, calculateOrderLiquidatePrice)
        when (mPosition.side) {

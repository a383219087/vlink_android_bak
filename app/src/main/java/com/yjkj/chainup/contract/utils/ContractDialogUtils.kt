...
object ContractDialogUtils {
     ...
     * 合约平仓买卖提示框
     */
    fun showNormalDialog(context: Context, titleKey: String? = "", submitKey: String? = "common_text_btnConfirm".localized(context), cancelKey: String? = "common_text_btnCancel".localized(context), content: String? = "",
     ...
     * 已实现盈亏明细提示框
     */
    fun showGainsBalanceTipsDialog(context: Context, fundingRate: String, fee: String, loss: String) {
        if (!checkActivity(context)) {
            return
        }
        TDialog.Builder((context as AppCompatActivity).supportFragmentManager)
                .setLayoutRes(R.layout.sl_dialog_gains_balance_tips)
                .setScreenWidthAspect(context, 0.8f)
                .setGravity(Gravity.CENTER)
                .setDimAmount(0.8f)
                .setCancelableOutside(false)
                .setOnBindViewListener {
                    it.setText(R.id.tv_title, "contract_text_realisedPNLDetail".localized(context))
                            .setText(R.id.tv_content, "contract_eranOrLosstips".localized(context))
                            .setText(R.id.tv_funding_fee, "sl_str_gains_balance_fee".localized(context))
                            .setText(R.id.tv_funding_fee_value, fundingRate)
                            .setText(R.id.tv_fee, "sl_str_fee".localized(context))
                            .setText(R.id.tv_fee_value, fee)
                            .setText(R.id.tv_gain_loss, "contract_text_gainOrLossOffset".localized(context))
                            .setText(R.id.tv_gain_loss_value, loss)
                            .setText(R.id.tv_confirm, "alert_common_i_understand".localized(context))
                }
                .addOnClickListener(R.id.tv_confirm)
                .setOnViewClickListener { _, view, tDialog ->
                    if(view.id == R.id.tv_confirm){
                        tDialog.dismiss()
                    }
                }
                .create()

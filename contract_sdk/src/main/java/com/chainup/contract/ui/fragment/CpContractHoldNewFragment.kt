...
class CpContractHoldNewFragment : CpNBaseFragment() {
                            ...
                            override fun bottonOnClick() {
                                ...
                                addDisposable(getContractModel().createOrder(obj, consumer = object : CpNDisposableObserver(true) {
                                    override fun onResponseSuccess(jsonObject: JSONObject) {
                                        ...
                                        mReverseOpenDialog?.dismiss() //开仓接口
                                        cancelOrder(clickData.contractId.toString(),true)
                                        addDisposable(getContractModel().createOrder(obj1,
    ...
    private fun showSwitch() {
        showAll = CpPreferenceManager.getInt(activity!!, CpPreferenceManager.isShowAllContract, 0)
        if (showAll == 0) {
                ...
                object : CpDialogUtil.DialogBottomListener {
                    override fun sendConfirm() {
                        ...
                        for (i in 0 until mList.size) {
                            ...
                            val mMultiplierPrecision = CpClLogicContractSetting.getContractMultiplierPrecisionById(context, clickData.contractId)
                            val mMultiplier = CpClLogicContractSetting.getContractMultiplierById(context, clickData.contractId)
                            val mPricePrecision = CpClLogicContractSetting.getContractSymbolPricePrecisionById(activity, clickData.contractId)
                            val num1 = CpBigDecimalUtils.mulStr(clickData.canCloseVolume, mMultiplier, mMultiplierPrecision)
    ...
    private fun cancelOrder(contractId:String,isisConditionOrder: Boolean) {
        addDisposable(getContractModel().orderCancelAll("[${contractId}]", isisConditionOrder, consumer = object : CpNDisposableObserver(activity, false) {
            override fun onResponseSuccess(jsonObject: JSONObject) {
